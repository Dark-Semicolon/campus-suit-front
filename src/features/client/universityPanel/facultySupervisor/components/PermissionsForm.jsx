...
function PermissionsForm({
    ...
    const groupedPermissions = groupPermissionsByPostfix(
        permissions?.data || []
    );
        ...
        <section className="flex flex-col items-center justify-center py-10">
            ...
            {isPending ? (
                ...
                <>
                    ...
                    >
                        <span className="font-bold text-blue-color-primary">
                            Select All Permissions
                        </span>
                    </Checkbox>
                    <div className="flex flex-wrap items-center justify-center w-full gap-12 py-10">
                        {Object.keys(groupedPermissions).map((postfix) => (
                            <div
                                key={postfix}
                                className="flex flex-col items-start justify-start gap-5 py-5"
                            >
                                <Checkbox
                                    color="primary"
                                    isSelected={groupedPermissions[
                                        postfix
                                    ].every((p) =>
                                        selectedPermissions?.includes(p.id)
                                    )}
                                    onChange={(e) =>
                                        handleSelectGroup(postfix, e)
                                    }
                                    isDisabled={isDisabled}
                                ...
                                >
                                    {groupedPermissions[postfix].map(
                                        (permission) => (
                                            <Checkbox
                                                key={permission.id}
                                                color="primary"
