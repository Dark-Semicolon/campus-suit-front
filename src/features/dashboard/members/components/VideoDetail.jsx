...
function VideoDetail({ myViews, userId, maxViews, videoId }) {
  ...
  const { userWatchedVideos, isPending } = useUserWatchedVideos({
    userId,
    filter,
  });
  ...
  function handelChange(e) {
    if (maxViewCount !== e.target.value) setMaxViewCount(e.target.value);
  }

  function onSubmit() {
    updateVideoView(
      { userId, WatchedVideoId, max_view_count: maxViewCount },
      {
        onSuccess: () => {
          setStartEdit(false);
        },
  ...
  return (
    <div className="flex flex-col justify-between h-full">
      <div className="space-y-5">
        <p>
          - عدد المرات التي شوهدت :
          <span className="px-1 text-gray-color-primary">{myViews}</span>
        </p>
        <div>
          <label>- أقصي حد للمشاهدة :</label>
          <input
      ...
      {!startEdit &&
        WatchedVideoId &&
        can("update:course:lecture:video:views") && (
          <Button
            type="primary"
      ...
      {startEdit &&
        WatchedVideoId &&
        can("update:course:lecture:video:views") && (
          <Button type="primary" onClick={onSubmit} disabled={isUpdating}>
            حفظ التعديل
          </Button>
        )}
    </div>
