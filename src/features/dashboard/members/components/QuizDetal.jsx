...
function QuizDetal({ userId, isSuccess }) {
    ...
    return (
        <div>
            ...
            <p>
                - عدد مرات دخول الأمتحان :<span className="px-1 text-gray-color-primary">{0}</span>
            </p>
            <div>
                <label>
                    - عدد المرات المتبيقة  :
                ...
                <input
                    ...
                    defaultValue={maxAttemptsCount} type="number"
                    disabled={!startEdit}
                    onChange={handelMaxAttemptsCountChange}
                />
            </div>
            {!startEdit && <Button type='primary' onClick={() => setStartEdit(true)} >
                تعديل
            </Button>}
            {startEdit && <Button type='primary'>
                حفظ التعديل
            </Button>}
        </div>
    )
