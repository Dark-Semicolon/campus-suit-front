  ...
  return (
    <div className="flex items-center justify-start w-30">
      <Avatar
        isBordered
        color="default"
        src={university?.data?.attributes?.logo === null ? "/images/userPlaceholder.png" : `${STORAGE_LINK}/${university?.data?.attributes?.logo}`}
        classNames={{ base: "w-12 h-12 ", img: "object-contain" }}
      />
      <h2 className="text-sm font-bold capitalize truncate text-blue-color-primary w-30 ps-2 w ">{university?.data?.attributes?.name}</h2>
    </div>
  );
}
