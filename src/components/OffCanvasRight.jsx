...
const Offcanvas = ({ id, children }) => {
    ...
    return (
        <>
            <button
                type="button"
                className="flex items-center justify-center p-4 text-sm font-semibold text-gray-800 border border-transparent rounded-full bg-yellow-color-primary size-7 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                data-hs-overlay={`#${id}`}
                onClick={toggleOpen}
            >
            ...
            <div
                id={id}
                className={`hs-overlay transition-all duration-300 transform fixed inset-0 ${isOpen ? 'translate-x-0' : 'translate-x-full'} right-0 top-0 start-0 max-w-xs w-full z-80 bg-white border-e dark:bg-gray-800 dark:border-gray-700`}
                tabIndex="-1"
...
export { OffcanvasProvider, Offcanvas, OffcanvasBody, OffcanvasCloseButton, OffcanvasTitle, OffcanvasHeader };
