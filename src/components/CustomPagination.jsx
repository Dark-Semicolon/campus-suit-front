            ...
            <ul className="flex items-center gap-2">
                {range.map((page) => {
                    if (page === PaginationItemType.NEXT) {
                        return (
                            <li key={page} aria-label="next page" className="text-center w-9 h-9">
                                <button
                    ...
                    if (page === PaginationItemType.PREV) {
                        return (
                            <li key={page} aria-label="previous page" className="text-center w-9 h-9">
                                <button
                                    className="flex items-center justify-center w-full h-full transition-all duration-75 rounded-xl text-small hover:bg-default-200"
                    ...
                    if (page === PaginationItemType.DOTS) {
                        return (
                            <li key={`${page}-${Math.random()}`} className="text-center w-9 h-9 text-gray-color-primary">
                                ...
                            </li>
                        );
                    }
                    ...
                    return (
                        <li key={page} aria-label={`page ${page}`} className="font-normal w-9 h-9 text-smal">
                            <button
                                className={cn(
                                    "w-full h-full rounded-xl text-small",
                                    activePage === page && "bg-yellow-color-primary",
                                    activePage !== page && "hover:bg-default-200 transition-all duration-75"
                                )}
