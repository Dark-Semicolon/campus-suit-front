...
//getUserUniversities
export async function getUserUniversities({ filter, fields, sortBy, page = 1, perPage = 10, paginationLink }) {
    await csrf();

    // Base API Query
    let queryLink = `${API_WEB}/universities`;
    // apiOperations
    const query = apiOperations({ queryLink, filter, fields, sortBy, page, perPage, paginationLink });
...
export async function updateUniversity({ logo, name, description, universityId }) {
    ...
    const response = await axios.patch(`${API_WEB}/universities/${universityId}`, { logo, name, description });
